...
function wechat_sendxml($url, $arr, $cert = false) {
	...
	$xml = wechat_xml($arr);
	if ($cert) {
		$cert_arr['ssl_cert'] = "{$pe['path_root']}public/plugin/payment/wechat/cert/apiclient_cert.pem";
		$cert_arr['ssl_key'] = "{$pe['path_root']}public/plugin/payment/wechat/cert/apiclient_key.pem";		
	}
...
function wechat_config($update = null) {
	...
	if (is_file("{$pe['path_root']}cache/wechat_config.cache.php")) {
	...
	if ($wechat_config['time'] <= time() - 3600 * 1.5 || $wechat_config['wechat_access_token'] == '' || $update) {
		...
		$wechat_config['notify_url'] = "{$pe['host_root']}public/plugin/payment/wechat/notify_url.php";
		$wechat_config['refund_url'] = "{$pe['host_root']}public/plugin/payment/wechat/refund_url.php";
...
/*function wechat_menuurl($match) {
	...
	if (stripos($match[1], trim($pe['host_root'], '/')) === false) {
		...
		return "https://open.weixin.qq.com/connect/oauth2/authorize?appid={$wechat_config['wechat_appid']}&redirect_uri=".urlencode($match[1])."&response_type=code&scope=snsapi_userinfo&state=xyshop_wechat#wechat_redirect";
	}
}*/

//绑定手机号
function bind_phone($phone,$openid){
	global $db;
...
function wechat_webpay($order_id) {
	...
	if ($json['return_code'] == 'SUCCESS' && $json['result_code'] == 'SUCCESS') {
		pe_lead('public/class/phpqrcode.class.php');
...
function wechat_h5pay($order_id) {
	...
	if ($json['return_code'] == 'SUCCESS' && $json['result_code'] == 'SUCCESS') {
		$url_arr = order_pay_goto($order_id, 0);
		$jump_url = $pe['host_root']."user.php?mod=order&act=view&id=".$order_id;
		$json['mweb_url'] .= "&redirect_url=".urlencode($jump_url);
...
function wechat_refund($refund_id) {
	...
	$xml_arr['appid'] = $wechat_config['wechat_pay_appid'];
	$xml_arr['mch_id'] = $wechat_config['wechat_mchid'];
	$xml_arr['nonce_str'] = md5(microtime(true).pe_ip().'myllop+dazhu');
...
//返回code地址
function wechat_get_code($redirect_url){
	$wechat_config = wechat_config();
	//$uristr = "{$pe['host_root']}index.php?mod=test&act=getuserinfo";
	$jumpurl = "https://open.weixin.qq.com/connect/oauth2/authorize?appid={$wechat_config['wechat_appid']}&redirect_uri=".urlencode($redirect_url)."&response_type=code&scope=snsapi_userinfo&state=xyshop_wechat#wechat_redirect";
	return $jumpurl;
}

//获取userinfo的access_token信息
function wechat_user_access_token($code){
	$wechat_config = wechat_config();
	$json = pe_curl_get("https://api.weixin.qq.com/sns/oauth2/access_token?appid={$wechat_config['wechat_appid']}&secret={$wechat_config['wechat_appsecret']}&code=".$code."&grant_type=authorization_code");
	return json_decode($json, true);
}

//获取userinfo用户信息
function wechat_userinfo($tokeninfo){
	if(is_array($tokeninfo)){
		$userinfo = pe_curl_get("https://api.weixin.qq.com/sns/userinfo?access_token=".$tokeninfo['access_token']."&openid=".$tokeninfo['openid']."&lang=zh_CN");
