...
function payment_list($type = 'order') {
	...
	foreach ($cache_payment as $k=>$v) {
		...
		elseif ($type == 'admin' && !in_array($v['payment_type'], array('cod', 'balance', 'bank'))) {
			$payment_list[$k]['payment_name'] = $v['payment_name'];		
			$payment_list[$k]['payment_type'] = $v['payment_type'];
		}*/
	}
...
function order_calback_add($order, $cart_id = array()) {
	...
	/*if ($info['order_virtual'] ==1){
		$product_info = $db->pe_select('orderdata', array('order_id'=>$info['order_id']));
		if ($prokey_num = $db->pe_num('prokey', array('product_id'=>$product_info['product_id']))){
			$sql = "select * from `".dbpre."prokey` where `user_id`=0 and `order_id`=0 and `product_id`=".$product_info['product_id']." limit 0,1";
			$prokey = $db->sql_select($sql);
			$sql_set['order_id'] = $info['order_id'];
			$sql_set['user_id'] = $info['user_id'];
			$sql_set['user_name'] = $info['user_name'];
			if ($prokey['prokey_type']=='one'){
				
				
			}else{
				$db->pe_update('prokey', array('product_id'=>$product_info['product_id']), pe_dbhold($sql_set));
			}
		}
...
function order_callback_close($order, $order_closetext = '', $refund = true) {
	...
	if ($db->pe_update('order', array('order_id'=>$info['order_id']), pe_dbhold($sql_set))) {
		if ($info['order_pstate'] && $refund) {
			...
			//已付款订单进行退款操作
			$order_datalist = $db->pe_selectall('orderdata', array('order_id'=>$info['order_id']));
			foreach ($order_datalist as $v) {
				refund_success($v['refund_id']);
			}
...
function order_callback_virtual($order) {
	...
	$product = $db->pe_select('prokey', array('product_id'=>$info['product_id']), 'prokey_type, prokey_user, prokey_pw, prokey_edate');
	...
	if ($product['prokey_type'] == 'one') {
		...
		//检查prokey表中是否已经存在当前订单的信息
		$prokey_count = $db->pe_num('prokey', array('product_id'=>$info['product_id'], 'prokey_state'=>'success','order_id'=>$info['order_id'],'user_id'=>$info['user_id']));
		if($prokey_count < $info['product_num']){
