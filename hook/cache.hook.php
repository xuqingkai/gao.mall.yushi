...
function cache_write($cache_type = 'all') {
	...
	if (in_array($cache_type, array('category', 'all', 'data'))) {
		...
		cache::write('category', $db->index('category_id')->pe_selectall('category', array('order by'=>'`category_order` asc, `category_id` asc'), 'category_id,category_pid,category_name,category_icon'));
		cache::write('category_arr', $db->index('category_pid|category_id')->pe_selectall('category', array('order by'=>'`category_order` asc, `category_id` asc'), 'category_id,category_pid,category_name,category_icon'));
	...
	if (in_array($cache_type, array('template', 'all'))) {
		pe_dirdel("{$pe['path_root']}cache/template");
	}
	if (in_array($cache_type, array('attachment', 'all'))) {
		pe_dirdel("{$pe['path_root']}cache/attachment");
	}
	if (in_array($cache_type, array('thumb', 'all'))) {
		pe_dirdel("{$pe['path_root']}cache/thumb");
	}
}
?>