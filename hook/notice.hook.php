...
function add_noticelog($user_id, $type, $info = '') {
	...
	if ($notice['user']['notice_sms_state'] && $info['user_phone']) {
		...
		if(isset($_SESSION['sms_param'])){
			...
			if (qunfa_sms($info['user_phone'],$sms_param,$notice['user']['notice_sms_templateid'])){
	...
	if ($notice['admin']['notice_sms_state'] && $cache_setting['sms_admin']) {
		foreach (explode(',', $cache_setting['sms_admin']) as $v) {
			...
			if(isset($_SESSION['sms_param'])){
				...
				if (qunfa_sms($cache_setting['sms_admin'],$sms_param,$notice['admin']['notice_sms_templateid'])){
					$info_list['noticelog_state'] = 'success';
				}else{
					$info_list['noticelog_state'] = 'fail';
				}
