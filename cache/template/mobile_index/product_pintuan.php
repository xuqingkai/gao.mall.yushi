...
<div class="content">
	...
	<div class="pro_info" style="position:relative;border-top:0">
		...
		<div class="jg_price mat5">
			<span class="fl"><span class="font14 mar5">¥</span><?php echo $info['product_money'] ?></span>
			<span class="pt_rs fl"><?php echo $info['product_ptnum'] ?>人拼团价</span>
			<div class="clear"></div>
		</div>
	...
	<div class="pt_box">
		<div class="pt_box1">
			<div class="pt_box_tt">拼团玩法<!--<a href="#">玩法详情<i class="jt_r" style="right:0"></i></a>--></div>
			<div class="pt_info">
				<ul>
				<li><div><span class="pt_num">1</span>选择商品<p>付款开团</p></div></li>
				<li><div><span class="pt_num">2</span>邀请好友<p>付款参团</p></div></li>
				<li><div><span class="pt_num">3</span>24小时内<p>人满成团</p></div></li>
				</ul>
				<div class="clear"></div>
			</div>
		</div>
...
<?php if($pintuan_num):?>
<div class="pindan_box" id="gundong_html" style="height:180px; overflow:hidden;">
	<div class="pro_pjtt mar10">
		<a href="javascript:app_page('page_pintuan');"><?php echo $pintuan_num ?>人在拼团<span>查看更多</span><i></i></a>
	</div>
	<div class="gundong_list">
		<?php foreach($pintuan_list as $v):?>
		<div class="pindan_list">
			<div class="pindan_l">
				<span class="pindan_tx"><img src="<?php echo pe_thumb($v['user_logo'], '_120', '_120', 'avatar') ?>"></span>
				<span class="pindan_name"><?php echo pe_cut($v['user_name'], 6, '..') ?></span>
			</div>
			<div class="pindan_r">
				<p class="">
					还差<span class="cred"><?php echo $v['pintuan_num'] - $v['pintuan_joinnum'] ?></span>人<br />
					<span class="font12 c888">剩余 <span class="pintuan_time" endtime="<?php echo $v['pintuan_etime'] ?>"><span class="jstime_h"></span>:<span class="jstime_m"></span>:<span class="jstime_s"></span></span></span>
				</p>
				<a href="<?php echo pe_url('product-'.$info['product_id'], 'pintuan_id='.$v['pintuan_id']) ?>" class="pindan_btn">去拼团</a>
			</div>
		</div>
		<?php endforeach;?>
	</div>
...
<div class="pro_foot">
	...
	<div class="pro_foot_right">
		...
		<?php else:?>
		<a href="javascript:buy_open('buy');">¥<?php echo $info['product_smoney'] ?> 单买</a>
		<?php if($_g_pintuan_id):?>
		<a href="javascript:buy_open('pintuan');" class="gm">¥<?php echo $info['product_money'] ?> 参团</a>
		<?php else:?>
		<a href="javascript:buy_open('pintuan');" class="gm">¥<?php echo $info['product_money'] ?> 开团</a>		
		<?php endif;?>
		<?php endif;?>
		<div class="clear"></div>
	</div>
...
<div id="page_buy" class="divhide">
		...
		<div class="tc_pro">
			...
			<tr>
				...
				<td valign="top">
					...
					<p class="c888 mat10">单买：¥<?php echo $info['product_smoney'] ?></p>
					<p class="cred">拼团：¥<span id="product_money"><?php echo $info['product_money'] ?></span></p>
				</td>
	...
	<div class="buy_btn">
		...
		<?php else:?>
		<a href="javascript:buy_btn('buy', '<?php echo $info['product_id'] ?>');" type="buy">¥<?php echo $info['product_smoney'] ?> 单独购买</a>
		<?php if($_g_pintuan_id):?>
		<a href="javascript:buy_btn('pintuan', '<?php echo $info['product_id'] ?>');" type="pintuan" class="gm">¥<?php echo $info['product_money'] ?> 确认参团</a>
		<?php else:?>
		<a href="javascript:buy_btn('pintuan', '<?php echo $info['product_id'] ?>');" type="pintuan" class="gm">¥<?php echo $info['product_money'] ?> 确认开团</a>
		<?php endif;?>
		<?php endif;?>
		<div class="clear"></div>
	</div>
	...
	<div style="position:relative">
		<div class="share_box" id="share_html">
			...
			<div class="share_pro">
				...
				<div class="mat5 cred font16">拼团价：¥<?php echo $info['product_money'] ?></div>
				<div class="clear"></div>
			</div>
		</div>
...
<div id="page_pintuan" class="divhide">
	<div class="add_tt">本商品拼单列表</div>
	<div class="close_btn" onclick="app_page_close();"></div>
	<div class="pindan_more1">
		<div class="pindan_box">
			<?php foreach($pintuan_list as $v):?>
			<div class="pindan_list">
				<div class="pindan_l">
					<span class="pindan_tx"><img src="<?php echo pe_thumb($v['user_logo'], '_120', '_120', 'avatar') ?>"></span>
...
<div class="pindan_cy" style="display:none">
	<div class="cy_box">
		<div class="cy_tt">参与红果果的拼单</div>
		<div class="cy_text">仅剩<span class="cred">1</span>个名额，23：10：10后结束</div>
		<div class="pt_txlist">
			<ul>
			<li class="pt_zhu mal5"><img src="<?php echo pe_thumb($v['user_logo'], _50, _50, 'avatar') ?>"><span class="pt_zhu_text">拼主</span></li>
			<!--<li class="pt_qt mal5"><span class=""></span></li>-->
			</ul>
		</div>
		<div class="cy_btn"><a href="#">参与拼单</a></div>
	</div>
</div>
...
$(function(){
	...
	pe_jstime(".pintuan_time", '<?php echo time() ?>');
...
function buy_btn(act, id) {
	...
	app_getinfo("<?php echo $pe['host_root'] ?>index.php?mod=cart&act="+act+"&id="+id+"&guid="+$(":input[name='product_guid']").val()+"&num="+$(":input[name='product_num']").val()+"&pintuan_id=<?php echo $_g_pintuan_id ?>", function(json){
		if (json.result == true) {
			...
			/*if (act == 'buy') {
				...
				app_page_close();
				app_open("<?php echo $pe['host_root'] ?>index.php?mod=order&act=add_pintuan&ptid=<?php echo $order_ptid ?>&id="+json.id);
			}
			else {
...
wx.ready(function(){
	wx_share({
		...
		desc: "拼团价：<?php echo $info['product_money'] ?>元",
