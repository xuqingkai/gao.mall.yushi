...
foreach ($info_list as $v) {
	$o_info = $db->pe_select('order', array('order_id'=>$v['order_id']),'order_payment');
	
	if ($o_info['order_payment'] != 'wechat' && $o_info['order_payment'] != 'alipay'){
...
foreach ($info_list as $v) {
	...
	if($v['huodong_type']=='zhekou'){
		$db->pe_update('prodata', array('product_id'=>$v['product_id']), "product_money = `product_smoney` * $bb");
	}else{
