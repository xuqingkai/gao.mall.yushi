...
<div class="right">
	<div class="now">
		<a href="webadmin.php?mod=user" <?php if(!$_g_userlevel_id):?>class="sel"<?php endif;?>>会员列表（<?php echo $tongji['all'] ?>）<i></i></a>
		<?php foreach($cache_userlevel as $k=>$v):?>
		<a href="webadmin.php?mod=user&userlevel_id=<?php echo $k ?>" <?php if($_g_userlevel_id==$k):?>class="sel"<?php endif;?>><?php echo $v['userlevel_name'] ?>（<?php echo $tongji[$k] ?>）<i></i></a>		
		<?php endforeach;?>
		<div class="clear"></div>
	</div>
	<div class="right_main">
		...
		<tr>
			...
			<td class="num"><a href="webadmin.php?mod=order&user_id=<?php echo $v['user_id'] ?>" class="cblue" target="_blank"><?php echo $v['user_ordernum'] ?></a></td>
			...
			<td>
				<a href="webadmin.php?mod=user&act=edit&id=<?php echo $v['user_id'] ?>&<?php echo pe_fromto() ?>" class="admin_edit mar3">修改</a>
				<a href="webadmin.php?mod=user&act=del&id=<?php echo $v['user_id'] ?>&token=<?php echo $pe_token ?>" class="admin_del mar3" onclick="return pe_cfone(this, '删除')">删除</a>
				<a href="webadmin.php?mod=user&act=login&id=<?php echo $v['user_id'] ?>&token=<?php echo $pe_token ?>" class="admin_btn" target="_blank">登录</a>
			</td>
		</tr>
		<?php endforeach;?>
		</table>
	...
	<div class="right_bottom">
		<span class="fl mal10">
			...
			<button href="webadmin.php?mod=user&act=del&token=<?php echo $pe_token ?>" onclick="return pe_cfall(this, 'user_id', 'form', '批量删除')">批量删除</button>
...
$(function(){
	...
	$("#sendemail").click(function(){
		...
		url = 'webadmin.php?mod=user&act=email&id='+user_id.join(",");
