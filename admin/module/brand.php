...
switch ($act) {
	...
	case 'add':
		if (isset($_p_pesubmit)) {
			pe_token_match();
			pe_lead('public/class/pinyin.class.php');
			$pinyin = new pinyin();
			...
			if ($brand_id = $db->pe_insert('brand', pe_dbhold($_p_info))) {
				if ($_FILES['brand_logo']['size']) {
					...
					$upload = new upload($_FILES['brand_logo'], 'attachment/brand/', array('filename'=>$brand_id));
					$db->pe_update('brand', array('brand_id'=>$brand_id), array('brand_logo'=>$upload->filehost));
				}
				cache_write('brand');
				pe_success('添加成功!', 'webadmin.php?mod=brand');
			}
			else {
				pe_error('添加失败!');
			}
		...
		include(pe_tpl('brand_add.html','admin'));
	break;
	//####################// 修改品牌 //####################//
	case 'edit':
		...
		if (isset($_p_pesubmit)) {
			...
			if ($db->pe_update('brand', array('brand_id'=>$brand_id), pe_dbhold($_p_info))) {
				cache_write('brand');
				pe_success('修改成功!', 'webadmin.php?mod=brand');
			}
			else {
				pe_error('修改失败...');
			}
	...
	default :
		...
		include(pe_tpl('brand_list.html','admin'));
	break;
}
?>